...
int main() {
  ...
  // GPS scatter of ~25 m (synthetic coordinates at the same latitude band) must stay
  // below MOVE_ALARM_M=150, and a genuine move of ~0.002 deg lat (~221 m) must trip it.
  float scatter = geoDistM(55.00113f, 9.00785f, 55.00131f, 9.00794f);
  assert(scatter < 150.0f);
  float moved = geoDistM(55.0011f, 9.0078f, 55.0031f, 9.0078f);
  assert(moved > 150.0f);
