...
| `gps_last`, `gps_last_s`, `gps_last_sats`, `gps_last_svs`, `gps_last_hdop` | outcome of the most recent GNSS hunt, in **every** frame (a hunt runs after the frame, so it describes the previous attempt): 1 = no fix, 2 = fix seen but rejected by the quality gate, 3 = published, 4 = good fix obtained but the follow-up frame could not be sent (it is kept in NVS and backfilled at its own timestamp on a later wake); seconds spent; most satellites in view during the window (fix or not) and their per-constellation split in `+CGNSSINFO` sentence order (this A7608E-H firmware reports three fields — GPS/GLONASS/BeiDou — and no Galileo field); best HDOP seen. `gnss_mode` is the engine's answer to the requested `+CGNSSMODE=15` (all constellations). |
| `vbat_rest_mv`, `vbat_min_mv`, `vbat_sag_ms`, `vbat_sag_at` | supply sag through the modem power-on, sampled at ~500 Hz on the cycles that survive: resting voltage with the modem rail off, the minimum seen, when (ms after rail-on) and in which step (1 cap precharge, 2 rail on, 3 reset pulse, 4 PWRKEY, 5 modem boot). `(rest − min) / ~2 A` ≈ the cell + holder path resistance; brownouts at `ph=1` mean it exceeds ~0.4 Ω. |
...
# scope the InfluxDB access (the admin token must not live in Telegraf or Grafana):
B=$(docker exec freezermon-influxdb-1 influx bucket list --name telemetry --hide-headers | cut -f1)
docker exec freezermon-influxdb-1 influx auth create --org freezermon --write-bucket "$B" --description telegraf-write   # -> TELEGRAF_INFLUX_TOKEN
docker exec freezermon-influxdb-1 influx auth create --org freezermon --read-bucket  "$B" --description grafana-read     # -> GRAFANA_INFLUX_TOKEN
# put both in .env, then: docker compose up -d telegraf grafana
```

Grafana: `http://<host>:3000` → **FreezerMon** folder → dashboard shows temperature, power source, door, battery, signal, and a location map. Firewall: expose only **8883** (devices, TLS — via Traefik or the `mosquitto.conf` TLS listener) and **3000** (you) — plaintext 1883 is bound to localhost and InfluxDB stays internal. The firmware's default build env is TLS; the plain env + `MQTT_PORT 1883` is for LAN-only bring-up.
...
A unit that has been silent for **more than a day** gets no further messages at all — no 4-hourly reminders, no "OK again" — until it reports again (the offline message at 15 min already told you it went quiet). The relay does the muting from a `last_report_age_s` annotation every rule carries, so Grafana's own alert state is untouched. Tune or disable with `MUTE_AFTER_SILENT_S` in `.env` (seconds, default 86400, `0` = never mute); the relay logs every muted message.
...
- Broker requires authentication **and authorization**: `mosquitto/config/acl` confines every identity to its own topics (per-device users via `pattern … %u`; the shared device credential can never write a `cmd` topic; Telegraf is read-only; `PublishFirmware.ts` uses a `publisher` identity that can only write `cmd`). A leaked device credential therefore cannot arm an OTA. Until per-device credentials land (HARDENING P3) the shared `freezer` identity can still write any unit's `telemetry`/`alert` topics — forging readings, and thereby clearing a breach alert, remains possible for whoever holds it — and, because each device subscribes to its own retained frame as delivery proof, the shared identity can also *read* every unit's telemetry, coordinates included. The stack refuses to start with unset secrets (`${VAR:?}`). The committed default is **plaintext 1883** (the `8883` listener is a commented upgrade path in `mosquitto.conf`, and `platformio.ini`'s default env is the non-TLS build) — for anything Internet-facing, enable 8883 and build `t-a7608-tls`; the fork's secure client encrypts but doesn't validate the broker cert by default, so signed-OTA is what guarantees firmware integrity regardless; for full MQTT MITM protection, load a CA cert via the modem's certificate store.
